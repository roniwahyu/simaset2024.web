...
use App\Exports\CorerolesListExport;
use App\Exports\CorerolesViewExport;
...
class CoreRolesController extends Controller
{
	...
	function store(CoreRolesAddRequest $request){
		...
		return $this->redirect("coreroles", "Record added successfully");
	}
	...
	function edit(CoreRolesEditRequest $request, $rec_id = null){
		...
		if ($request->isMethod('post')) {
			...
			return $this->redirect("coreroles", "Record updated successfully");
		}
	...
	private function ExportList($query){
		...
		$filename = "ListCoreRolesReport-" . date_now();
		$format = $this->getExportFormat();
		if($format == "print"){
			$records = $query->get(CoreRoles::exportListFields());
			return view("reports.coreroles-list", ["records" => $records]);
		}
		elseif($format == "pdf"){
			$records = $query->get(CoreRoles::exportListFields());
			$pdf = PDF::loadView("reports.coreroles-list", ["records" => $records]);
			return $pdf->download("$filename.pdf");
		}
		elseif($format == "csv"){
			return Excel::download(new CorerolesListExport($query), "$filename.csv", \Maatwebsite\Excel\Excel::CSV);
		}
		elseif($format == "excel"){
			return Excel::download(new CorerolesListExport($query), "$filename.xlsx", \Maatwebsite\Excel\Excel::XLSX);
		}
	}
	...
	private function ExportView($query, $rec_id){
		ob_end_clean();// clean any output to allow file download
		$filename ="ViewCoreRolesReport-" . date_now();
		$format = $this->getExportFormat();
		if($format == "print"){
			$record = $query->findOrFail($rec_id, CoreRoles::exportViewFields());
			return view("reports.coreroles-view", ["record" => $record]);
		}
		elseif($format == "pdf"){
			$record = $query->findOrFail($rec_id, CoreRoles::exportViewFields());
			$pdf = PDF::loadView("reports.coreroles-view", ["record" => $record]);
			return $pdf->download("$filename.pdf");
		}
		elseif($format == "csv"){
			return Excel::download(new CorerolesViewExport($query, $rec_id), "$filename.csv", \Maatwebsite\Excel\Excel::CSV);
		}
		elseif($format == "excel"){
			return Excel::download(new CorerolesViewExport($query, $rec_id), "$filename.xlsx", \Maatwebsite\Excel\Excel::XLSX);
		}
	}
}
