...
use App\Exports\RoomListExport;
...
class RoomController extends Controller
{
	...
	function store(RoomAddRequest $request){
		...
		$rec_id = $record->id;
		return $this->redirect("room", "Record added successfully");
	}
	...
	function edit(RoomEditRequest $request, $rec_id = null){
		...
		if ($request->isMethod('post')) {
			...
			return $this->redirect("room", "Record updated successfully");
		}
	...
	private function ExportList($query){
		...
		$filename = "ListRoomReport-" . date_now();
		$format = $this->getExportFormat();
		if($format == "print"){
			$records = $query->get(Room::exportListFields());
			return view("reports.room-list", ["records" => $records]);
		}
		elseif($format == "pdf"){
			$records = $query->get(Room::exportListFields());
			$pdf = PDF::loadView("reports.room-list", ["records" => $records]);
			return $pdf->download("$filename.pdf");
		}
		elseif($format == "csv"){
			return Excel::download(new RoomListExport($query), "$filename.csv", \Maatwebsite\Excel\Excel::CSV);
		}
		elseif($format == "excel"){
			return Excel::download(new RoomListExport($query), "$filename.xlsx", \Maatwebsite\Excel\Excel::XLSX);
		}
	}
}
